...
- 已根據線上截圖再次收斂账号頁第一行工具列。
- 修改內容：
  - `frontend/src/components/admin/account/AccountTableActions.vue`
  - `frontend/src/views/admin/AccountsView.vue`
- 修改前後差異：
  - 修改前：分組容器自身帶背景、邊框和陰影，與內部按鈕形成雙層膠囊，視覺噪音偏重。
  - 修改後：工具列只保留刷新、自動刷新、列設定、添加账号與「更多」入口；同步、匯入、匯出、去重、錯誤透傳、TLS 指紋等低頻操作收進「更多」下拉，避免第一行長按鈕互相擠壓。
  - 視覺樣式同步改為透明工具組、30px 高低噪音按鈕與輕量下拉菜單，新增按鈕保留主色突出。
- 影響範圍：
  - 僅調整账号管理頁第一行工具列樣式，不改動功能與資料流。
