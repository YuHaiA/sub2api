  ...
  admin: {
    ...
    accounts: {
      ...
      healthSummary: {
        total: '账号总数',
        healthy: '存活账号',
        healthyHint: '存活统计包含限流中的账号',
        bannedOrExhausted: '封禁或额度耗尽',
        bannedOrExhaustedHint: '包含封禁、402、额度耗尽等状态',
        unavailable: '检测异常',
    ...
    settings: {
      ...
      tabs: {
        ...
        deploy: {
          title: '部署更新',
          description: '配置 Git 仓库同步部署，并通过宿主机脚本重建 Docker 容器。',
          enabled: '启用部署模式',
          enabledHint: '启用后，后台“立即部署更新”和服务器手动脚本都会走同一套 Git 同步部署链路。',
          agentHintTitle: '宿主机负责执行部署',
          agentHintBody: 'Sub2API 仍然继续运行在 Docker 中，只是由宿主机脚本执行 git pull、docker build 和 docker-compose up，避免容器内直接操作宿主机失败。',
          repoUrl: '仓库地址',
          repoUrlPlaceholder: '例如 https://github.com/YuHaiA/sub2api.git',
          branch: '分支',
          branchPlaceholder: '例如 main',
          repoDir: '源码目录',
          repoDirPlaceholder: '例如 /home/ec2-user/sub2api-source',
          agentUrl: '代理地址',
          ...
          manualCommandHint: '将宿主机脚本复制到 /home/ec2-user/sub2api-deploy/bin/deploy-from-git.sh 后，可直接在服务器执行这条命令。',
          status: '部署状态',
