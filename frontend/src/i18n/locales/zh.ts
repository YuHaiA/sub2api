export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      deduplicateAction: '清理重复账号',
      deduplicateRunning: '清理中...',
      deduplicateConfirm: '按当前筛选范围清理重复账号：同一平台 + 类型 + 名称只保留最早一条，其余将被删除。',
      deduplicateConfirmButton: '确认清理',
      deduplicateCompleted: '重复账号清理完成：删除 {deleted} 条，涉及 {groups} 组重复',
      deduplicateFailed: '清理重复账号失败',
      healthCheckAll: '批量检测存活',
      healthCheckSelected: '检测选中账号',
      healthCheckRunning: '检测中...',
      healthCheckCompleted: '已完成 {count} 个账号的存活检测',
      healthCheckFailed: '账号存活检测失败',
      ...
      autoCheck: '自动测活',
      autoCheckEnabled: '启用自动测活',
      autoCheckInterval: '检测间隔（分钟）',
      autoCheckIntervalHint: '最小 1 分钟，建议 30 分钟以上',
      ...
      tokenRefresh: {
        ...
        scopeGroup: '指定分组',
        group: '目标分组',
        groupPlaceholder: '请选择要刷新的分组',
        groupRequired: '请选择要刷新的分组',
        save: '保存自动刷新配置',
        saved: '自动刷新 Token 配置已保存',
        runNow: '立即刷新一次',
        running: '刷新中...',
        runStarted: '已开始后台刷新，统计结果稍后自动更新',
        runAlreadyRunning: '后台刷新正在执行中，请稍后再看结果',
        runningProgress: '后台刷新进行中，进度会自动更新',
        lastRunAt: '最近刷新：{time}',
        lastRunTotal: '上次扫描',
        lastRunSuccess: '上次成功',
        lastRunFailed: '上次失败',
      ...
      healthSummary: {
        total: '账号总数',
        healthy: '存活账号',
        healthyHint: '仅统计单纯存活账号',
        constrained: '额度耗尽或限流',
        constrainedHint: '包含限流、402、额度耗尽等受限状态',
        unavailable: '异常账号',
    ...
    settings: {
      ...
      tabs: {
        ...
        deploy: {
          title: '部署更新',
          description: '配置 GitHub 镜像包部署，并通过宿主机脚本更新 Docker 容器。',
          enabled: '启用部署模式',
          enabledHint: '启用后，后台“立即部署更新”和服务器手动脚本都会走同一套镜像包部署链路。',
          agentHintTitle: '宿主机负责执行部署',
          agentHintBody: 'Sub2API 仍然继续运行在 Docker 中，只是由宿主机脚本下载 GitHub 最新镜像包并执行 docker load、docker tag 和 docker-compose up，避免容器内直接操作宿主机失败。',
          archiveUrl: '镜像包地址',
          archiveUrlPlaceholder: '例如 GitHub Release 上的 tar 下载地址',
          loadedImage: '镜像包内镜像名',
          loadedImagePlaceholder: '例如 sub2api-gha:docker-deploy',
          defaultImage: '目标镜像标签',
          defaultImagePlaceholder: '例如 weishaw/sub2api:latest',
          agentUrl: '代理地址',
          agentUrlPlaceholder: '例如 http://172.17.0.1:18080',
          agentToken: '代理令牌',
          agentTokenPlaceholder: '留空表示不校验 Bearer Token',
          agentTimeout: '代理超时（秒）',
          agentInsecureTls: '允许不校验证书',
          agentInsecureTlsHint: '仅在代理使用自签名 HTTPS 证书时开启。',
          serviceName: '服务名',
          composeProjectDir: 'Compose 项目目录',
          composeFile: 'Compose 文件路径',
          composeFilePlaceholder: '留空表示使用项目目录下默认 docker-compose.yml',
          manualCommand: '手动部署命令',
