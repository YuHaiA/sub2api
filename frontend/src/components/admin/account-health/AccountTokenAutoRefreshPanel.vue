<template>
  <div class="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(380px,460px)]">
    <div class="grid self-start gap-4 sm:grid-cols-2">
      ...
      <div class="min-h-[158px] rounded-2xl border border-sky-200 bg-sky-50/90 p-5 dark:border-sky-900/40 dark:bg-sky-900/10">
        <p class="text-xs font-medium uppercase tracking-wide text-sky-700 dark:text-sky-300">{{ t('admin.accounts.tokenRefresh.currentBatch') }}</p>
        <p class="mt-3 text-3xl font-semibold text-sky-700 dark:text-sky-200">{{ tokenConfig.batch_size }}</p>
        <p class="mt-2 text-xs text-sky-600/80 dark:text-sky-300/80">
