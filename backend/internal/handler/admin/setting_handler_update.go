...
// settingKeyJSONAliases covers request fields whose JSON name differs from
// the setting key they persist to. Every other value-typed field uses its JSON
// name as the storage key.
var settingKeyJSONAliases = map[string]string{
	"smtp_from_email": service.SettingKeySMTPFrom,
}

// settingKeyByJSONName maps value-typed top-level request fields to the keys
// they write. Pointer fields already distinguish omission from an explicit
// zero value and keep their existing merge semantics.
...
// A partial whole-document PUT must leave those stored values unchanged.
