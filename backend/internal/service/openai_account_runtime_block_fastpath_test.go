...
func TestShouldStopOpenAIOAuth429Failover_TracksTwoGrokFollowupAttempts(t *testing.T) {
	svc := &OpenAIGatewayService{}
	...
	t.Run("429 allows two failing followups", func(t *testing.T) {
		var state OpenAIOAuth429FailoverState
		require.False(t, svc.ShouldStopOpenAIOAuth429Failover(account, http.StatusTooManyRequests, 1, &state))
	...
	t.Run("500 then 429 still allows two followups", func(t *testing.T) {
		var state OpenAIOAuth429FailoverState
		require.False(t, svc.ShouldStopOpenAIOAuth429Failover(account, http.StatusInternalServerError, 1, &state))
		...
		require.True(t, svc.ShouldStopOpenAIOAuth429Failover(account, http.StatusServiceUnavailable, 4, &state))
	})

	t.Run("OAuth 429 then API-key failures consume the same budget", func(t *testing.T) {
		var state OpenAIOAuth429FailoverState
		require.False(t, svc.ShouldStopOpenAIOAuth429Failover(account, http.StatusTooManyRequests, 1, &state))
		require.False(t, svc.ShouldStopOpenAIOAuth429Failover(apiKeyAccount, http.StatusInternalServerError, 2, &state))
		require.True(t, svc.ShouldStopOpenAIOAuth429Failover(apiKeyAccount, http.StatusBadGateway, 3, &state))
	})

	t.Run("legacy caller stops after the third Grok 429", func(t *testing.T) {
		require.False(t, svc.ShouldStopOpenAIOAuth429Failover(account, http.StatusTooManyRequests, 2, nil))
		require.True(t, svc.ShouldStopOpenAIOAuth429Failover(account, http.StatusTooManyRequests, 3, nil))
	})
