...
func TestAPIKeyService_RejectsV17AuthSnapshotBeforeGrokDefaultImageBackfill(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-pre-grok-image-backfill", &APIKeyAuthCacheEntry{
	...
	if ok {
		t.Fatal("expected v17 auth snapshot to be rejected after the Grok image-access backfill")
	}
	if apiKey != nil {
